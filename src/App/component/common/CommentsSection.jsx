            ...
            <List.Item
              ...
              actions={[
                <div key={"client-comments"} className="mb-3 d-flex justify-content-start align-items-center gap-3">
                  ...
                  /> */}
                  <span className="text-danger">{item.createdBy?._id === auth?.user?._id && <DeleteOutlined onClick={() => deleteComment(item._id)} />}</span>
                </div>,
              ]}
            >
              <List.Item.Meta
                title={<a className="text-dark text-capitalize">{item?.createdBy?._id === auth?.user?._id ? "Me" : item?.createdBy?.role}</a>}

                description={[<span className="text-dark">{item?.content}</span>]}
              />
            </List.Item>
          )}
        />
