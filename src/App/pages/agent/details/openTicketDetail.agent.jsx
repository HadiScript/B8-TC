...
import { useAvailableAgents, useSingleTicket } from "../../../../actions/agent";
...
const AgentOpenTicketDetail = () => {
  ...
  const { loading, ticket: ticketData, EscTicket, closeTicket, doComment, deleteComment, comment,
    setComment, } = useSingleTicket(id);
  ...
  return (
    <>
      ...
      <div className="d-flex justify-content-end gap-2 mb-5">
        ...
        {
          !path.includes("escalated/ticket") && <Button onClick={AlertEsc} className="myBtn">
            Escalate Ticket
          </Button>
        }
      ...
      <Modal centered width={700} title={<span className="text-capitalize">{ticketData?.title}</span>} open={openEscModal} onCancel={() => setOpenEscModal(false)} footer={null}>
        <div className="mt-4">
        ...
        <div className="mt-4">
          {
            ...
            usersList
              ?.filter((x) => x._id !== auth?.user?._id).length > 0 ?
              usersList
      ...
      <CommentsSection list={ticketData?.comments} loading={loading} deleteComment={deleteComment} doComment={doComment} comment={comment} setComment={setComment} />
